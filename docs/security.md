...
| **Destructive actions** — losing mail | No permanent-delete tool exists in any configuration. The most destructive operation is a reversible move to Trash; emptying it stays in the Proton apps. Every mutation is audited. |
...
Every mutation — drafts, organize operations, attachment saves, sends and
send *attempts* — is appended to `~/.mcp-proton-email/audit.log` (directory
`0700`, file `0600`), rotated at 5 MB. Entries record the tool, outcome,
account, and for drafts/sends the recipients and subject line — intentional
accountability — but **never message bodies**.
